module Admin
  class MatchVideoTimestampsController < Admin::ApplicationController
    ...
    # Accepts video_links: { <video_link_id> => { starts_at_seconds: "34" } }
    def update
      match = Match.friendly.find(params[:match_id])
      ...
      VideoLink.transaction do
        updates.each do |id, attrs|
          ...
          value = attrs[:starts_at_seconds].to_s.strip
          new_seconds = value.empty? ? nil : Integer(value)
          if link.starts_at_seconds != new_seconds
            link.update!(starts_at_seconds: new_seconds)
            saved += 1
          end
        end
      end
